...
void MultiOrderBinaryThreePhasesDrivingForceStochioB::addDrivingForce(
   ...
   for (int kk = kmin; kk <= kmax; kk++) {
      for (int jj = jmin; jj <= jmax; jj++) {
         for (int ii = imin; ii <= imax; ii++) {
            ...
            const double epsilon1 = 2.e-4;
            const double epsilon2 = 3.e-4;
            double factor = 1.;
            if (hphiB >= 1. - epsilon1) {
               factor = 0.;
            } else {
               if (hphiB >= 1. - epsilon2) {
                  ...
                  double h = (hphiB - (1. - epsilon1)) / (epsilon1 - epsilon2);
