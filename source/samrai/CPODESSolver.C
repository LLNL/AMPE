...
int CPODESSolver::CPODESRHSFuncEval(realtype t, N_Vector y, N_Vector y_dot,
                                    void* my_solver, int fd_flag)
{
   return ((CPODESSolver*)my_solver)
       ->getCPODESFunctions()
       ->evaluateRHSFunction(t, SABSVEC_CAST(y), SABSVEC_CAST(y_dot), fd_flag);
}

int CPODESSolver::CPODESProjEval(realtype t, N_Vector y, N_Vector corr,
                                 realtype epsProj, N_Vector err,
                                 void* my_solver)
{
   return ((CPODESSolver*)my_solver)
       ->getCPODESFunctions()
...
int CPODESSolver::CPSpgmrPrecondSet(realtype t, N_Vector y, N_Vector fy,
                                    int jok, booleantype* jcurPtr,
                                    ...
                                    N_Vector vtemp3)
{
   ...
   int success;
   success =
       ((CPODESSolver*)my_solver)
           ->getCPODESFunctions()
           ->CPSpgmrPrecondSet(t, SABSVEC_CAST(y), SABSVEC_CAST(fy), jok,
                               jcurPtr, gamma, SABSVEC_CAST(vtemp1),
                               SABSVEC_CAST(vtemp2), SABSVEC_CAST(vtemp3));

   return (success);
}

int CPODESSolver::CPSpgmrPrecondSolve(realtype t, N_Vector y, N_Vector fy,
                                      N_Vector r, N_Vector z, realtype gamma,
                                      realtype delta, int lr, void* my_solver,
                                      N_Vector vtemp)
{
   int success;
   success = ((CPODESSolver*)my_solver)
                 ->getCPODESFunctions()
                 ->CPSpgmrPrecondSolve(t, SABSVEC_CAST(y), SABSVEC_CAST(fy),
                                       SABSVEC_CAST(r), SABSVEC_CAST(z), gamma,
                                       delta, lr, SABSVEC_CAST(vtemp));
...
*************************************************************************
*/
CPODESSolver::CPODESSolver(const std::string& object_name,
...
#pragma warning(disable : 1572)
   ...
   if (d_CPODE_needs_initialization) {
      ...
      N_Vector y0 =
          ((void*)d_ic_vector != NULL) ? d_ic_vector->getNVector() : NULL;
      ...
      if (d_tolerance_type == "vector") {
         ...
         ierr = CPodeSStolerances(d_cpode_mem, d_relative_tolerance,
      ...
      if (d_iteration_type == CP_NEWTON) {
         ...
         ierr = CPSpgmr(d_cpode_mem, d_precondition_type, d_max_krylov_dim);
         CPODE_SAMRAI_ERROR(ierr);
...
int CPODESSolver::solve()
{
    ...
    */
   retval = CPode(d_cpode_mem, d_user_t_f, &d_actual_t_f,
                  d_solution_vector->getNVector(),
                  d_solution_deriv_vector->getNVector(), d_stepping_method);
...
void CPODESSolver::setLogFileData(const std::string& log_fname)
{
...
void CPODESSolver::setCPODESFunctions(CPODESAbstractFunctions* my_functions,
                                      const bool uses_preconditioner)
{
...
{
   TBOX_ASSERT((tolerance_type == "scalar") || (tolerance_type == "vector"));
...
void CPODESSolver::setFinalValueOfIndependentVariable(
    double t_f, bool cpode_needs_initialization)
{
   d_user_t_f = t_f;
...
void CPODESSolver::setMaximumLinearMultistepMethodOrder(int max_order)
{
...
void CPODESSolver::setMaximumNumberOfInternalSteps(int max_num_internal_steps)
{
...
void CPODESSolver::setMaximumNumberOfNilStepWarnings(int max_num_warnings)
{
...
{
   TBOX_ASSERT(
       (precondition_type == PREC_NONE) || (precondition_type == PREC_LEFT) ||
       (precondition_type == PREC_RIGHT) || (precondition_type == PREC_BOTH));
...
void CPODESSolver::printCPODESStatistics(std::ostream& os) const
{
   ...
   sprintf(buf, "lenrw           = %5d     leniw            = %5d\n",
           getCPODESMemoryUsageForDoubles(), getCPODESMemoryUsageForIntegers());
   os << buf;
...
void CPODESSolver::printClassData(std::ostream& os) const
{
   ...
   os << "d_linear_multistep_method = " << d_linear_multistep_method
      << std::endl;
   os << "d_iteration_type = " << d_iteration_type << std::endl;
   os << "d_tolerance_type = " << d_tolerance_type << std::endl;
   ...
   os << "Optional CPODES inputs (see CPODES docs for details):" << std::endl;
   ...
   os << std::endl;
   os << "d_nonlin_conv_coef = " << d_nonlin_conv_coef << std::endl;
