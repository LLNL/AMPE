...
void ParabolicFreeEnergyMultiOrderBinaryThreePhase::computeFreeEnergy(
    ...
    Thermo4PFM::PhaseIndex pi, const double energy_factor)
{
   ...
   assert(energy_factor > 0.);
   assert(d_parabolic_fenergy);
   ...
   for (int kk = kmin; kk <= kmax; kk++) {
      for (int jj = jmin; jj <= jmax; jj++) {
         for (int ii = imin; ii <= imax; ii++) {
            ...
            ptr_f[idx_f] = d_parabolic_fenergy->computeFreeEnergy(t, &c_i, pi);
            ptr_f[idx_f] *= energy_factor;
            assert(!std::isnan(ptr_f[idx_f]));
         }
      }
   }
}
...
void ParabolicFreeEnergyMultiOrderBinaryThreePhase::addDrivingForceOnPatch(
