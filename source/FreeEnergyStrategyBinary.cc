...
FreeEnergyStrategyBinary::FreeEnergyStrategyBinary(
      ...
      d_with_third_phase(with_third_phase)
{
   ...
   // tbox::plog << "FreeEnergyStrategyBinary:" << std::endl;
...
void FreeEnergyStrategyBinary::computeDerivFreeEnergyLiquid(
...
void FreeEnergyStrategyBinary::computeDerivFreeEnergySolidA(
...
void FreeEnergyStrategyBinary::computeDerivFreeEnergySolidB(
    hier::Patch& patch, const int temperature_id, const int dfs_id)
{
   assert(temperature_id >= 0.);
   assert(dfs_id >= 0);
...
void FreeEnergyStrategyBinary ::computeFreeEnergyLiquid(
...
void FreeEnergyStrategyBinary ::computeFreeEnergySolidA(
...
void FreeEnergyStrategyBinary::computeFreeEnergySolidB(hier::Patch& patch,
...
void FreeEnergyStrategyBinary::computeFreeEnergy(
...
void FreeEnergyStrategyBinary::computeDerivFreeEnergy(
   ...
   for (int kk = kmin; kk <= kmax; kk++) {
      for (int jj = jmin; jj <= jmax; jj++) {
         for (int ii = imin; ii <= imax; ii++) {
            ...
            ptr_f[idx_f] = computeFreeEnergy(t, &c_i, pi, gp);
         }
      }
   }
}
   ...
   for (int kk = kmin; kk <= kmax; kk++) {
      for (int jj = jmin; jj <= jmax; jj++) {
         for (int ii = imin; ii <= imax; ii++) {
            ...
            ptr_f[idx_f] = computeDerivFreeEnergy(t, &c_i, pi);
         }
      }
   }
}
...
void FreeEnergyStrategyBinary::addDrivingForce(
