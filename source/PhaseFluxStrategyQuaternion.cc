...
void PhaseFluxStrategyQuaternion::computeFluxes(
    ...
    const int quat_id, const int flux_id)
{
   ...
   for (hier::PatchLevel::Iterator ip(level->begin()); ip != level->end();
      ...
      for (int i = 0; i < phase->getDepth() - 1; i++)
         QUATERNION_GRADIENT_FLUX(ifirst(0), ilast(0), ifirst(1), ilast(1),
#if (NDIM == 3)
                                  ifirst(2), ilast(2),
#endif
                                  dx, d_epsilon_phase, d_nu, d_knumber,
                                  phase->getPointer(i),
                                  phase->getGhostCellWidth()[0], &d_quat[i][0],
                                  4, phase_flux->getPointer(0, i),
...
#endif
                             dx, d_epsilon_phase,
                             phase->getPointer(phase->getDepth() - 1),
                             phase->getGhostCellWidth()[0],
                             phase_flux->getPointer(0, phase->getDepth() - 1),
                             phase_flux->getPointer(1, phase->getDepth() - 1),
#if (NDIM == 3)
                             phase_flux->getPointer(2, phase->getDepth() - 1),
#endif
                             phase_flux->getGhostCellWidth()[0]);
