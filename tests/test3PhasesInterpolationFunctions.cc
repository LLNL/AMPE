...
int main(int argc, char *argv[])
{
   ...
   cout << "Test Moelans2011..." << endl;
   {
      double val = TiltingMoelans2011::g0(1., 0., 0.);
      if (fabs(val - 1.) > tol) {
         cerr << "Test failed!" << endl;
         return 1;
      }
      ...
      double phi = 0.2;
      double val0 = TiltingMoelans2011::g0(1. - phi, phi, 0.);
      double val1 = 1. - TiltingMoelans2011::g0(phi, 1. - phi, 0.);
      if (fabs(val1 - val0) > tol) {
         cerr << "Test failed: g0(1-phi,phi,0)!=1-g0(phi,1-phi,0)!" << endl;
         return 1;
      }
    ...
    */
   {
      ...
      // compare with finite difference
      double eps = 1.e-8;
      double val0 = TiltingMoelans2011::g0(p0, p1, p2);
      double val1 = TiltingMoelans2011::g0(p0 + 2. * eps / 3., p1 - eps / 3.,
                                           p2 - eps / 3.);
      double deriv0 = (val1 - val0) / eps;
      double deriv1 = TiltingMoelans2011::dg0dp0(p0, p1, p2);
      cout << "Moelans2011 FD deriv=" << deriv0
           << ", Analytical deriv=" << deriv1
           << ", Difference =" << deriv1 - deriv0 << std::endl;
      if (fabs(deriv1 - deriv0) > tol) {
         return 1;
      }

      val0 = TiltingMoelans2011::g0(p0, p1, p2);
      val1 = TiltingMoelans2011::g0(p0 - eps / 3., p1 + 2. * eps / 3.,
                                    p2 - eps / 3.);
      deriv0 = (val1 - val0) / eps;
      deriv1 = TiltingMoelans2011::dg0dp1(p0, p1, p2);
      cout << "Moelans2011 FD deriv=" << deriv0
           << ", Analytical deriv=" << deriv1
           << ", Difference =" << deriv1 - deriv0 << std::endl;
      if (fabs(deriv1 - deriv0) > tol) {
