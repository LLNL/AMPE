...
#pragma warning (disable:1572)
   ...
   if (d_CVODE_needs_initialization) {
      ...
      int ierr = CVodeInit(d_cvode_mem, RHSFunc, d_t_0, y0);
      ...
      if(d_tolerance_type == "vector") {
         ierr = CVodeSVtolerances(d_cvode_mem, d_relative_tolerance,
                                  d_absolute_tolerance_vector -> getNVector());
      } else {
         ierr = CVodeSStolerances(d_cvode_mem, d_relative_tolerance, d_absolute_tolerance_scalar);
      }
      ...
      d_CVODE_needs_initialization = (y0 == NULL);
