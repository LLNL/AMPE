...
int main()
{
  ...
  while(t<Tout) {
    ...
    ad = NV_Ith_S(yy,1);
    E  = ad*ad - 2*g*sin(a);
    CPodeGetLastOrder(cpode_mem, &qu);
    printf("%le  %14.10le  %14.10le     %14.10le     %d\n",  t, a, ad, E, qu);
  }
  ...
  */
  ...
  while(t<Tout) {
    ...
    E  = ad*ad - 2*g*sin(a);
    CPodeGetLastOrder(cpode_mem, &qu);
    CPodeGetLastStep(cpode_mem, &hu);
    printf("%le  %14.10le  %14.10le     %14.10le   %d",  t, a, ad, E, qu);
    printf("        %e\n",hu*sqrt(g*sin(a)));
  }
...
static int cfun(realtype t, N_Vector yy, N_Vector cout, void *c_data)
{
  ...
  NV_Ith_S(cout,0) = ad*ad - 2*g*sin(a);
