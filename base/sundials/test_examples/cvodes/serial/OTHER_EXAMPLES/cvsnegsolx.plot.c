...
/* With these tolerances, the solution blows up once it
   becomes negative!!! */
/*
#define RTOL  RCONST(1.0e-4)
#define ATOL1 RCONST(7.0e-7)
...
int main()
{
  ...
  fout1 = fopen("no_cntrl.out","w");
  fout2 = fopen("cntrl.out","w");
  ...
  while(1) {
    ...
    iout++;
    tout *= TMULT;
    if (iout == NOUT-2) break;
  }

  /* continue in ONE_STEP to save data for plotting */
  tout *= TMULT;
  CVodeSetStopTime(cvode_mem, tout);
  while (t<tout) {
    CVode(cvode_mem, tout, y, &t, CV_ONE_STEP_TSTOP);
    fprintf(fout1,"%14.8e  %14.8e %14.8e  %14.8e\n",t,Ith(y,1),Ith(y,2),Ith(y,3));
    printf("%14.8e  %14.8e %14.8e  %14.8e\n",t,Ith(y,1),Ith(y,2),Ith(y,3));
  }
  ...
  while (t<tout) {
    CVode(cvode_mem, tout, y, &t, CV_ONE_STEP_TSTOP);
    fprintf(fout2,"%14.8e  %14.8e %14.8e  %14.8e\n",t,Ith(y,1),Ith(y,2),Ith(y,3));
  }
...
static int f(realtype t, N_Vector y, N_Vector ydot, void *f_data)
{
  ...
  /*
  if ((y1<0 || y2<0 || y3<0) ) {
    printf("aha  %14.6le  %14.6le %14.6le  %14.6le \n",t,y1,y2,y3);
  }
  */
