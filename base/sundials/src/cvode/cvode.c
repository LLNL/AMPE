          ...
          realtype *tret, int itask)
{
  ...
  if (nst == 0) {
    ...
    retval = f(tn, zn[0], zn[1], user_data, 0); 
    nfe++;
    if (retval < 0) {
...
static int CVYddNorm(CVodeMem cv_mem, realtype hg, realtype *yddnrm)
{
  ...
  retval = f(tn+hg, y, tempv, user_data, 0);
  nfe++;
  if (retval < 0) return(CV_RHSFUNC_FAIL);
...
static int CVNlsFunctional(CVodeMem cv_mem)
{
  ...
  retval = f(tn, zn[0], tempv, user_data, 0);
  nfe++;
  if (retval < 0) return(CV_RHSFUNC_FAIL);
  if (retval > 0) return(RHSFUNC_RECVR);
  ...
  loop {
    ...
    retval = f(tn, y, tempv, user_data, 0);
    nfe++;
    if (retval < 0) return(CV_RHSFUNC_FAIL);
    if (retval > 0) return(RHSFUNC_RECVR);
...
static int CVNlsNewton(CVodeMem cv_mem, int nflag)
{
  ...
  loop {
    ...
    retval = f(tn, zn[0], ftemp, user_data, 0);
    nfe++; 
    if (retval < 0) return(CV_RHSFUNC_FAIL);
    if (retval > 0) return(RHSFUNC_RECVR);
...
static int CVNewtonIteration(CVodeMem cv_mem)
{
  ...
  loop {
    ...
    /* Save norm of correction, evaluate f, and loop again */
    delp = del;
    retval = f(tn, y, ftemp, user_data, 0);
    nfe++;
    if (retval < 0) return(CV_RHSFUNC_FAIL);
    if (retval > 0) {
      if ((!jcur) && (setupNonNull)) return(TRY_AGAIN);
