                        ...
                        N_Vector yC, N_Vector ypC, N_Vector fctC)
{
  ...
  retval = SpgmrSolve(spgmr_mem, cp_mem, x, b, pretype, gstype, delta, 0,
                      ...
                      &res_norm, &nli_inc, &nps_inc, cp_mem->cp_print_diagnostics);
  ...
  if (retval != SPGMR_SUCCESS && cp_mem->cp_print_diagnostics) printf("Linear iteration failed with return value = %d\n", retval);
