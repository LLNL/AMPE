...
static int cpNlsFunctionalExpl(CPodeMem cp_mem)
{
  ...
  retval = fe(tn, zn[0], tempv, user_data, 0);
  nfe++;
  ...
  loop {
    ...
    /* Evaluate f again */
    retval = fe(tn, y, tempv, user_data, 0);
    nfe++;
    if (retval < 0) return(CP_ODEFUNC_FAIL);
    if (retval > 0) return(ODEFUNC_RECVR);
...
static int cpNlsNewtonExpl(CPodeMem cp_mem, int nflag)
{
  ...
  if (lsetup_exists) {      
     ...
     */
    callSetup = (nflag == PREV_CONV_FAIL) || (nflag == PREV_ERR_FAIL) ||
      (nst == 0) || (nst >= nstlset + cp_mem->cp_max_precond_steps) || (ABS(gamrat-ONE) > DGMAX);
  } else {  
    crate = ONE;
    callSetup = FALSE;
  }
  ...
  loop {
    ...
    retval = fe(tn, zn[0], ftemp, user_data, 0);
    nfe++; 
    if (retval < 0) return(CP_ODEFUNC_FAIL);
    if (retval > 0) return(ODEFUNC_RECVR);
    ...
    if (ier != CP_SUCCESS && cp_mem->cp_print_diagnostics) printf("Newton iteration failed\n");
...
static int cpNewtonIterationExpl(CPodeMem cp_mem)
{
  ...
  loop {
    ...
    /* Save norm of correction, evaluate f, and loop again */
    delp = del;
    retval = fe(tn, y, ftemp, user_data, 0);
    nfe++;
    if (retval < 0) return(CP_ODEFUNC_FAIL);
    if (retval > 0) return(ODEFUNC_RECVR);
