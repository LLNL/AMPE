...
int CPodePrintDiagnostics(void *cpode_mem, booleantype enable)
{
  CPodeMem cp_mem;

  if (cpode_mem==NULL) {
    cpProcessError(NULL, CP_MEM_NULL, "CPODES", "CPodePrintDiagnostics", MSGCP_NO_MEM);
    return(CP_MEM_NULL);
  }
  cp_mem = (CPodeMem) cpode_mem;

  cp_mem->cp_print_diagnostics = enable;

  return(CP_SUCCESS);
}

int CPodeSetMaxPrecondSteps(void *cpode_mem, int max_steps)
{
  CPodeMem cp_mem;
  ...
  cp_mem->cp_max_precond_steps = max_steps;
